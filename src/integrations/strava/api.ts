...
async function refreshTokens(
  ...
  await writeJson(env.STATE, STRAVA_API_STATE_KEYS.tokens, refreshed);
  return refreshed;
}
...
export async function exchangeStravaCode(
  ...
  await writeJson(env.STATE, STRAVA_API_STATE_KEYS.tokens, tokens);
  return tokens;
}
...
async function handleAuthorize(request: Request, context: RouteContext): Promise<Response> {
  ...
  await env.STATE.put(STRAVA_API_STATE_KEYS.oauthState, state, { expirationTtl: 600 });
...
async function handleCallback(request: Request, context: RouteContext): Promise<Response> {
  ...
  const expectedState = await env.STATE.get(STRAVA_API_STATE_KEYS.oauthState);
  ...
  await env.STATE.delete(STRAVA_API_STATE_KEYS.oauthState);
